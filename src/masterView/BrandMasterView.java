...
public class BrandMasterView extends javax.swing.JInternalFrame {
    ...
    public BrandMasterView(int formCD) {
        initComponents();
...
//    }
                        ...
                        protected Object doInBackground() throws Exception {
                            ...
                            if (row != -1) {
                                try {
                                    ...
                                    lb.removeGlassPane(BrandMasterView.this);
                                }
                            }
    ...
    public void getData() {
        Call<JsonObject> call = brandAPI.getBrandMaster(SkableHome.db_name,SkableHome.selected_year);
            ...
            public void onResponse(Call<JsonObject> call, Response<JsonObject> rspns) {
                ...
                if (rspns.isSuccessful()) {
                    ...
                    if (result.get("result").getAsInt() == 1) {
                        TypeToken<List<BrandMasterModel>> token = new TypeToken<List<BrandMasterModel>>() {
                        };
                        ArrayList<BrandMasterModel> detail = new Gson().fromJson(result.get("data"), token.getType());
                        dtm.setRowCount(0);
                        for (int i = 0; i < detail.size(); i++) {
                            Vector row = new Vector();
                            row.add(detail.get(i).getBRAND_CD());
