...
import masterController.GroupMasterController;
...
public class GroupMasterView extends javax.swing.JInternalFrame {
    ...
    public GroupMasterView(int fromCD) {
        initComponents();
...
//    }
    ...
    private void showDailogeAdd(String brand_cd, String brand_name, Object head_grp, int acc_eff) {
        GroupMasterController bmc = new GroupMasterController(null, true, this, brand_cd, brand_name, (head_grp == null) ? "" : head_grp.toString(), acc_eff);
                ...
                if (navLoad.getModel().getEDITS().equalsIgnoreCase("1")) {
                    ...
                    if (row != -1) {
                        ...
                        if (jTable1.getValueAt(row, 2) != null) {
                            lb.confirmDialog("Do you want to Edit This Group?");
                            if (lb.type) {
                                showDailogeAdd(jTable1.getValueAt(row, 0).toString(), jTable1.getValueAt(row, 1).toString(), jTable1.getValueAt(row, 2).toString(),
                                        Integer.parseInt(jTable1.getValueAt(row, 3).toString()));
                            }
                        }
                    }
                } else {
                        ...
                        protected Object doInBackground() throws Exception {
                            ...
                            if (row != -1) {
                                try {
                                    ...
                                    lb.removeGlassPane(GroupMasterView.this);
                                }
                            }
                ...
                if (rspns.isSuccessful()) {
                    ...
                    if (result.get("result").getAsInt() == 1) {
                        ...
                        for (int i = 0; i < detail.size(); i++) {
                            Vector row = new Vector();
                            row.add(detail.get(i).getGRP_CD());
                            row.add(detail.get(i).getGROUP_NAME());
                            row.add(detail.get(i).getHEAD_GRP());
                            row.add(detail.get(i).getACC_EFF());
                            dtm.addRow(row);
                        }
                    } else {
