...
import masterController.RefByController;
...
public class RefMasterView extends javax.swing.JInternalFrame {

    private DefaultTableModel dtm = null;
    private SmallNavigation navLoad = null;
    private final Library lb = Library.getInstance();
    private RefralAPI smAPI;
    private TableRowSorter<TableModel> rowSorter;
    private JTextField jtfFilter = new JTextField();

    public RefMasterView(int formCD) {
        initComponents();
        smAPI = lb.getRetrofit().create(RefralAPI.class);
        connectToNavigation();
        searchOnTextFields();
...
//    }
    ...
    private void showDailogeAdd(String sm_cd, String sm_name) {
        RefByController bmc = new RefByController(null, true, this, sm_cd, sm_name);
        bmc.setLocationRelativeTo(null);
        bmc.show();
    }

    public void addRow(String sm_cd, String sm_name) {
        ...
        if (selRow == -1) {
            ...
            jTable1.setValueAt(sm_cd, selRow, 0);
            jTable1.setValueAt(sm_name, selRow, 1);
        }
    }
        ...
        class navigation extends SmallNavigation {
            ...
            public void callEdit() {
                if (navLoad.getModel().getEDITS().equalsIgnoreCase("1")) {
                    ...
                    if (row != -1) {
                        navLoad.setMode("E");
                        lb.confirmDialog("Do you want to Edit This Salesman?");
                        if (lb.type) {
                        ...
                        protected Object doInBackground() throws Exception {
                            ...
                            if (row != -1) {
                                try {
                                    ...
                                    lb.printToLogFile("Exception at callDelete at Salesmanmaster", ex);
                                } finally {
                                    lb.removeGlassPane(RefMasterView.this);
                                }
                            }
    ...
    public void getData() {
        Call<JsonObject> call = smAPI.getReferalMaster(SkableHome.db_name,SkableHome.selected_year);
            ...
            public void onResponse(Call<JsonObject> call, Response<JsonObject> rspns) {
                ...
                if (rspns.isSuccessful()) {
                    ...
                    if (result.get("result").getAsInt() == 1) {
                        TypeToken<List<RefModel>> token = new TypeToken<List<RefModel>>() {
                        };
                        ArrayList<RefModel> detail = new Gson().fromJson(result.get("data"), token.getType());
                        dtm.setRowCount(0);
                        for (int i = 0; i < detail.size(); i++) {
                            Vector row = new Vector();
                            row.add(detail.get(i).getREF_CD());
                            row.add(detail.get(i).getREF_NAME());
                            dtm.addRow(row);
                        }
                    } else {
