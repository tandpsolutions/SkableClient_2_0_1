...
import masterController.CameraMasterController;
import model.CameraMasterModel;
...
public class CameraMasterView extends javax.swing.JInternalFrame {
    ...
    public CameraMasterView(int formCD) {
        initComponents();
...
//        add(new JScrollPane(jTable1), BorderLayout.CENTER);
    ...
    private void showDailogeAdd(String camera_cd, String camera_name) {
        CameraMasterController bmc = new CameraMasterController(null, true, this, camera_cd, camera_name);
        bmc.setLocationRelativeTo(null);
        bmc.setVisible(true);
    }

    public void addRow(String camera_cd, String camera_name) {
        ...
        if (selRow == -1) {
            ...
            jTable1.setValueAt(camera_cd, selRow, 0);
            jTable1.setValueAt(camera_name, selRow, 1);
        }
    }
        ...
        class navigation extends SmallNavigation {
            ...
            public void callEdit() {
                if (navLoad.getModel().getEDITS().equalsIgnoreCase("1")) {
                    ...
                    if (row != -1) {
                        navLoad.setMode("E");
                        lb.confirmDialog("Do you want to Edit This Camera?");
                        if (lb.type) {
                        ...
                        protected Object doInBackground() throws Exception {
                            ...
                            if (row != -1) {
                                try {
                                    ...
                                    lb.printToLogFile("Exception at callDelete at CameraMasterView", ex);
                                } finally {
                                    lb.removeGlassPane(CameraMasterView.this);
                                }
                            }
            ...
            public void onResponse(Call<JsonObject> call, Response<JsonObject> rspns) {
                ...
                if (rspns.isSuccessful()) {
                    ...
                    if (result.get("result").getAsInt() == 1) {
                        TypeToken<List<CameraMasterModel>> token = new TypeToken<List<CameraMasterModel>>() {
                        };
                        ArrayList<CameraMasterModel> detail = new Gson().fromJson(result.get("data"), token.getType());
                        dtm.setRowCount(0);
                        for (int i = 0; i < detail.size(); i++) {
                            Vector row = new Vector();
                            row.add(detail.get(i).getCAMERA_CD());
