...
import model.MemoryMasterModel;
...
public class MemoryMasterView extends javax.swing.JInternalFrame {
    ...
    public MemoryMasterView(int formCd) {
        initComponents();
...
//    }
    ...
    private void showDailogeAdd(String memory_cd, String memory_name) {
        MemoryMasterController bmc = new MemoryMasterController(null, true, this, memory_cd, memory_name);
        bmc.setLocationRelativeTo(null);
        bmc.show();
    }

    public void addRow(String color_cd, String color_name) {
        ...
        if (selRow == -1) {
            ...
            jTable1.setValueAt(color_cd, selRow, 0);
            jTable1.setValueAt(color_name, selRow, 1);
        }
    }
        ...
        class navigation extends SmallNavigation {
            ...
            public void callEdit() {
                if (navLoad.getModel().getEDITS().equalsIgnoreCase("1")) {
                    ...
                    if (row != -1) {
                        navLoad.setMode("E");
                        lb.confirmDialog("Do you want to Edit This Color?");
                        if (lb.type) {
                        ...
                        protected Object doInBackground() throws Exception {
                            ...
                            if (row != -1) {
                                try {
                                    ...
                                    lb.removeGlassPane(MemoryMasterView.this);
                                }
                            }
            ...
            public void onResponse(Call<JsonObject> call, Response<JsonObject> rspns) {
                ...
                if (rspns.isSuccessful()) {
                    ...
                    if (result.get("result").getAsInt() == 1) {
                        TypeToken<List<MemoryMasterModel>> token = new TypeToken<List<MemoryMasterModel>>() {
                        };
                        ArrayList<MemoryMasterModel> detail = new Gson().fromJson(result.get("data"), token.getType());
                        dtm.setRowCount(0);
                        for (int i = 0; i < detail.size(); i++) {
                            Vector row = new Vector();
                            row.add(detail.get(i).getMEMORY_CD());
