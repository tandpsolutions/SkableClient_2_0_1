...
import masterController.SalesmanController;
...
public class SalesmanMaster extends javax.swing.JInternalFrame {

    private DefaultTableModel dtm = null;
    private SmallNavigation navLoad = null;
    private final Library lb = Library.getInstance();
    private SalesmanAPI smAPI;
    private TableRowSorter<TableModel> rowSorter;
    private JTextField jtfFilter = new JTextField();

    public SalesmanMaster(int formCD) {
        initComponents();
        smAPI = lb.getRetrofit().create(SalesmanAPI.class);
        connectToNavigation();
        searchOnTextFields();
...
//    }
    ...
    private void showDailogeAdd(String sm_cd, String sm_name) {
        SalesmanController bmc = new SalesmanController(null, true, this, sm_cd, sm_name);
                        ...
                        protected Object doInBackground() throws Exception {
                            ...
                            if (row != -1) {
                                try {
                                    ...
                                    lb.removeGlassPane(SalesmanMaster.this);
                                }
                            }
            ...
            public void onResponse(Call<JsonObject> call, Response<JsonObject> rspns) {
                ...
                if (rspns.isSuccessful()) {
                    ...
                    if (result.get("result").getAsInt() == 1) {
                        TypeToken<List<SalesManMasterModel>> token = new TypeToken<List<SalesManMasterModel>>() {
                        };
                        ArrayList<SalesManMasterModel> detail = new Gson().fromJson(result.get("data"), token.getType());
                        dtm.setRowCount(0);
                        for (int i = 0; i < detail.size(); i++) {
                            Vector row = new Vector();
                            row.add(detail.get(i).getSMCD());
                            row.add(detail.get(i).getSMNAME());
                            dtm.addRow(row);
                        }
                    } else {
