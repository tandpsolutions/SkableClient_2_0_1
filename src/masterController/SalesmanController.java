...
public class SalesmanController extends javax.swing.JDialog {
    ...
    private void validateVoucher() {
        ...
        if (sm_cd.equalsIgnoreCase("")) {
            Call<JsonObject> call = lb.getRetrofit().create(SupportAPI.class).validateData("smmst", "sm_cd", "sm_name", jtxtSmName.getText(),SkableHome.db_name,SkableHome.selected_year);
            ...
            Call<JsonObject> call = lb.getRetrofit().create(SupportAPI.class).ValidateDataEdit("smmst", "sm_Cd", "sm_name", jtxtSmName.getText(), "sm_cd", sm_cd,SkableHome.db_name,SkableHome.selected_year);
            call.enqueue(new Callback<JsonObject>() {
                @Override
                public void onResponse(Call<JsonObject> call, Response<JsonObject> rspns) {
                    if (rspns.isSuccessful()) {
