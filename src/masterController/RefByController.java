...
public class RefByController extends javax.swing.JDialog {
    ...
    private RefMasterView smv = null;
    private RefralAPI salesmanAPI;

    public RefByController(java.awt.Frame parent, boolean modal, RefMasterView smv, String sm_cd, String sm_name) {
        super(parent, modal);
        initComponents();
        ...
        this.smv = smv;
        salesmanAPI = lb.getRetrofit().create(RefralAPI.class);
    ...
    private void validateVoucher() {
        if (lb.isBlank(jtxtSmName)) {
            lb.showMessageDailog("Refral name can not be left blank");
            jtxtSmName.requestFocusInWindow();
            return;
        }
                ...
                public void onResponse(Call<JsonObject> call, Response<JsonObject> rspns) {
                    if (rspns.isSuccessful()) {
                        if (rspns.body().get("result").getAsInt() == 0) {
                            lb.showMessageDailog("Refral already exist");
                            return;
                        } else {
                            saveVoucher();
                        }
    ...
    private void saveVoucher() {
        ...
        call.enqueue(new Callback<JsonObject>() {
            @Override
            public void onResponse(Call<JsonObject> call, Response<JsonObject> rspns) {
                lb.removeGlassPane(RefByController.this);
                if (rspns.isSuccessful()) {
                    if (rspns.body().get("result").getAsInt() == 1) {
                        lb.showMessageDailog(rspns.body().get("Cause").getAsString());
                        if (smv != null) {
                            smv.addRow(rspns.body().get("REF_CD").getAsString(), jtxtSmName.getText());
                        }
                        RefByController.this.dispose();
                    } else {
                        lb.showMessageDailog(rspns.body().get("Cause").getAsString());
                    }
                } else {
