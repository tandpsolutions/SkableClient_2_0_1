 ...
 * @author bhaumik
 */
public class DNCNView extends javax.swing.JInternalFrame {
    ...
    private DNCNApi dncnAPI;
    private int vType;

    public DNCNView(int type, int formCd) {
        initComponents();
        dncnAPI = lb.getRetrofit().create(DNCNApi.class);
...
//                        lb.showMessageDailog(response.body().getd("Cause").getAsString());
            ...
            Logger.getLogger(DNCNView.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
            ...
            public void callPrint() {
                if (navLoad.getModel().getPRINTS().equalsIgnoreCase("1")) {
                    ...
                    if (row != -1) {
                        ...
                        pp.getDnCnPrint(jTable1.getValueAt(row, 0).toString(), vType);
                    }
                } else {
                    lb.showMessageDailog("You don't have rights to perform this action");
                }
