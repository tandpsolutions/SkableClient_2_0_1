...
public class PurchaseView extends javax.swing.JInternalFrame {
    ...
    public void setData() {
        ...
        Call<PurchaseHead> call = purchasAPI.getDataHeader(lb.ConvertDateFormetForDB(jtxtFromDate.getText()), lb.ConvertDateFormetForDB(jtxtToDate.getText()), vType + "",
                (jcmbBranch.getSelectedIndex() == 0) ? "0" : Constants.BRANCH.get(jcmbBranch.getSelectedIndex()-1).getBranch_cd(), tax_type + "",SkableHome.db_name,SkableHome.selected_year);
        call.enqueue(new Callback<PurchaseHead>() {
            @Override
            public void onResponse(Call<PurchaseHead> call, Response<PurchaseHead> response) {
                lb.removeGlassPane(PurchaseView.this);
                if (response.isSuccessful()) {
                    ...
                    if (header.getResult() == 1) {
                        ...
                        for (int i = 0; i < header.getPurchaseHeader().size(); i++) {
                            ...
                            row.add(lb.getBRanchName(header.getPurchaseHeader().get(i).getBRANCHCD()+""));
                            dtm.addRow(row);
                        }
                ...
                if (navLoad.getModel().getDELETES().equalsIgnoreCase("1")) {
                    ...
                    if (row != -1) {
                        ...
                        if (lb.type) {
                            ...
                            purchasAPI.DeletePurchaseBill(ref_no,SkableHome.db_name,SkableHome.selected_year).enqueue(new Callback<JsonObject>() {
                                @Override
                    ...
                    if (row != -1) {
                        ...
                        call.enqueue(new Callback<JsonObject>() {
                            ...
                            public void onResponse(Call<JsonObject> call, Response<JsonObject> response) {
                                ...
                                if (response.isSuccessful()) {
//                                    System.out.println(response.body().toString());
                                    TagPrint tp = new TagPrint(response.body(), purchasAPI);
...
//            lb.printToLogFile("Exception at dispose at codeBinding", ex);
    ...
    private void initComponents() {
        ...
        jTable1.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "ref no", "INV No", "Bill No", "Type", "Date", "Name", "Net Amt", "Remark", "Ac Cd", "Branch"
            }
        ) {
