...
public class PurchaseReturnView extends javax.swing.JInternalFrame {
    ...
    private PurchaseReturnAPI purchaeReturnAPI;
    private int vType;

    public PurchaseReturnView(int type, int formCd) {
        initComponents();
        vType = type;
        purchaeReturnAPI = lb.getRetrofit().create(PurchaseReturnAPI.class);
                ...
                if (navLoad.getModel().getDELETES().equalsIgnoreCase("1")) {
                    ...
                    if (row != -1) {
                        ...
                        if (lb.type) {
                            ...
                            purchaeReturnAPI.DeletePurchaseReturn(ref_no).enqueue(new Callback<JsonObject>() {
                                @Override
