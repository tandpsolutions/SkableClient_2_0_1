 ...
 * @author Lenovo
 */
public class SelectAccount extends javax.swing.JDialog {
     ...
     */
    public SelectAccount(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
...
//        add(new JScrollPane(jTable1), BorderLayout.CENTER);
    ...
    public void fillData(ArrayList<AccountMaster> result) {
        if (!result.isEmpty()) {
            for (int i = 0; i < result.size(); i++) {
                Vector row = new Vector();
                row.add(result.get(i).getFNAME() + " " + result.get(i).getMNAME() + " " + result.get(i).getLNAME());
                row.add(result.get(i).getBAL() + "");
                row.add(result.get(i).getMOBILE1() + "");
                row.add(result.get(i).getTIN() + "");
                row.add(result.get(i).getADD1() + "");
                row.add(result.get(i).getACALIAS() + "");
                dtm.addRow(row);
            }
            Library.getInstance().setColumnSizeForTable(jTable1, jPanel1.getWidth());
        }
    }
    ...
    private void initComponents() {
        ...
        jTable1.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Account Name", "Bal", "Mobile", "Tin No", "Address", "Alias"
            }
        ) {
